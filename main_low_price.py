...
SHEET_NAME = "Low Price"  # Sheet tab name
    ...
    try:
        ...
        if username and password and username != "your_username":
            ...
            for i, pid in enumerate(player_ids):
                ...
                # Verify filter
                status = "OK" if buy_price < 1000000 else "FILTER FAIL"
                print(f"[{i+1}/{len(player_ids)}] ID: {pid} | Est: {est_val:,} | Buy: {buy_price:,} | Diff: {value_diff:,} | ROI: {roi:.1f}% [{status}]")
